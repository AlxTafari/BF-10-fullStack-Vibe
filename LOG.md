...
## 2026-09-02 (4) — Правки по код-ревью

**Цель:** закрыть находки ревью (`/code-review high`).

**Сделано:**
- Парсер сумм: `1,000` / `1.000` / `1 000` больше не превращаются в `1`.
  Новый `parseAmount`: пробел/апостроф — только разряды; один разделитель +
  ровно 3 цифры после = разряды; одинаковые разделители, группы по 3 = разряды
  («1.234.567» = 1234567); иначе последний — десятичный. Тесты добавлены.
- Верхняя граница суммы `1e12` — абсурдные числа перед валютой игнорируются.
- Регэксп терминов (`TERM_RE`) вынесен на уровень модуля — собирается один раз,
  а не на каждое сообщение.
- Гейтинг по типу чата: в группах бот реагирует только на слэш-команды,
  свободный текст — только в личке (`IncomingMessage.chatType`, `mapUpdate`
  прокидывает `chat.type`). Убирает спам курсами в ответ на обычные фразы
  и ложные срабатывания однословных синонимов («реал», «бат», «try»).
- `formatPopularRates`: пустой список курсов при валидном снимке — это не
  «источник недоступен», отдельный текст `POPULAR_RATES_EMPTY`.
- `POPULAR_CODES` — одна ассерция на массив вместо `.map(c => c as ...)`.

**Проверено:** tsc чисто, 37/37. README — блок про поведение в группах.
