...
В группах бот отвечает только на слэш-команды (`/rates`, `/source`, `/help`).
Свободный текст разбирается лишь в личке — иначе бот засыпал бы группу курсами
в ответ на обычные сообщения («на реал не хватает», «в 2024 EUR подорожал»).
