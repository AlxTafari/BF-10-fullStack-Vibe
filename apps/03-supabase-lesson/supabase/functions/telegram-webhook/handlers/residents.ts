...
export async function handleResidents(ctx: HandlerContext, user: UserRow): Promise<void> {
  ...
  if (residents.length === 0) {
    await reply(ctx, user.id, "За последние 30 дней в деревне никого не видели.");
    return;
  }

  const lines = residents.map(
    (r) => `${r.user.name ?? "безымянный"} — ${r.campName ?? "без лагеря"}, сплетен: ${r.newsCount}`,
  );
  await reply(ctx, user.id, `👥 Активные жители деревни:\n\n${lines.join("\n")}`);
}
